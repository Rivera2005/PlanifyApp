...
function crearElementoTarea(tarea, index) {
  ...
  const fechaConColor = colorFechaMostrar(fechaAMostrar, tarea.fecha); // Me devuelve un elemento p
...
// Función que, dependiendo del texto (Hoy/Mañana/día de semana/fecha) y la fecha original,
// asigna la clase correcta. Corrige el parseo para evitar el desfase UTC.
function colorFechaMostrar(textoFecha, fechaOriginal) {
  const p = document.createElement("p");
  p.textContent = textoFecha;

  // Intentar clasificar por fecha real primero (si viene la fecha original)
  if (fechaOriginal) {
    const partes = fechaOriginal.split("-").map(Number); // esperado: "YYYY-MM-DD"
    if (partes.length === 3 && partes.every((n) => !Number.isNaN(n))) {
      const [anio, mes, dia] = partes;

      // Medianoche local de hoy y de la fecha seleccionada
      const hoy = new Date();
      hoy.setHours(0, 0, 0, 0);

      // ¡Clave! Construimos la fecha en LOCAL (no con string ISO)
      const fechaSeleccionada = new Date(anio, mes - 1, dia);
      fechaSeleccionada.setHours(0, 0, 0, 0);

      const tSel = fechaSeleccionada.getTime();
      const tHoy = hoy.getTime();

      if (tSel < tHoy) {
        p.className = "claseVencida";
        return p;
      }
      if (tSel === tHoy) {
        // Si es hoy, asegúrate de que no se marque como vencida
        p.className = "claseHoy";
        return p;
      }
      // Si es futuro, seguimos con la lógica textual de abajo
    }
  }

  // Lógica textual (fallback) si no hay fechaOriginal o no se pudo parsear
  if (textoFecha === "Hoy") {
    p.className = "claseHoy";
